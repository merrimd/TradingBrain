...
namespace TradingBrain.Models
{
    public class MainApp
    {
        ...
        public MainApp(Database db, Container container, Container chart_container, string epic, Container _minute_container, Container _TicksContainer, Container _trade_container)
        {
            try
            {
                ...
                Console.WriteLine(DateTime.Now.ToString("o") + " - Model Run ID = " + modelID);
                ...
                if (v != null)
                {
                    ...
                    if (igWebApiConnectionConfig != null)
                    {
                        ...
                        //_l1TradeSubscription = new TradeTableListerner();
                        //_l1TradeSubscription.Update += OnTradeUpdate;
                        _tradeSubscription = new TradeSubscription(this);
                        _l1ChartSubscription = new ChartTickTableListerner();
                        _l1ChartSubscription.Update += OnTickUpdate;

                        //string[] epics = { "CS.D.USDJPY.TODAY.IP", "CS.D.EURUSD.TODAY.IP", "IX.D.FTSE.DAILY.IP", "IX.D.DAX.DAILY.IP", "IX.D.NASDAQ.CASH.IP" };
                        ...
                        clsCommonFunctions.SendBroadcast("Status", "running");

                        //Place an experimental deal
                        //PlaceDeal("long",0.5,42);
                        //PlaceDeal("short", 0.5, 42);
                        // Below needs to be uncommented to start the timer
                        ...
                        t.AutoReset = false;
                        t.Elapsed += new System.Timers.ElapsedEventHandler(RunCode);
        ...
        {
            try {
                TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "PlaceDeal", "Placing deal");
        ...
        {
            try {
                TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "CloseDeal", "Closing deal");
            ...
            IgResponse<ClosePositionResponse> ret = await igRestApiClient.closePosition(pos);
                ...
                if (ret != null)
                {
                    Console.WriteLine("Close deal - " + direction + " - Status: " + ret.StatusCode  );
                    TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "CloseDeal", "Close deal - " + direction + " - Status: " + ret.StatusCode  );
                }
            }
            catch (Exception e)
            {
        ...
        {
            try {
                TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "EditDeal", "Editing deal - " + dealID);
            ...
            IgResponse<EditPositionResponse> ret = await igRestApiClient.editPositionV1(dealID, pos);
                ...
                if (ret != null)
                {
                    Console.WriteLine("Edit deal - Status: " + ret.StatusCode  );
                    TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "EditDeal", "Edit deal - Status: " + ret.StatusCode  );
                }
        ...
        async void RunCode(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString("o") + " - Run Started ");
            Console.WriteLine(DateTime.Now.ToString("o") + " ------------------");
            ...
            this.tb = await clsCommonFunctions.GetTradingBrainSettings(this.the_db, this.epicName);
            //watch.Stop();
            ...
            model.thisModel.counterVar = this.tb.runDetails.counterVar;
            ...
            currentStatus.countervar = tb.runDetails.counterVar;
            ...
            while (_startTime <= _endTime)
            {
                ...
                //watch.Stop();
                //Console.WriteLine(DateTime.Now.ToString("o") + " - GetInputs - Time taken = " + watch.ElapsedMilliseconds);

                //Create the current candle
                //watch.Restart();
                model.candles.currentCandle = await CreateLiveCandle(the_db, thisInput.var1, thisInput.var3, thisInput.var2, thisInput.var13, _startTime, epicName, minute_container, TicksContainer, false, liveMode);
                //watch.Stop();
                //Console.WriteLine(DateTime.Now.ToString("o") + " - CreateLiveCandle - Time taken = " + watch.ElapsedMilliseconds);
                ...
                {
                    model.candles.prevCandle = await CreateLiveCandle(the_db, thisInput.var1, thisInput.var3, thisInput.var2, thisInput.var13, _startTime.AddMinutes(-1), epicName, minute_container, TicksContainer, false, false);

                }
                if (model.candles.prevCandle2.candleStart == DateTime.MinValue)
                {
                    model.candles.prevCandle2 = await CreateLiveCandle(the_db, thisInput.var1, thisInput.var3, thisInput.var2, thisInput.var13, _startTime.AddMinutes(-2), epicName, minute_container, TicksContainer, false, false);
                }



                //watch.Restart();
                DateTime getStartDate = model.getPrevMAStartDate(model.candles.currentCandle.candleStart);
                ...
                model.candles.prevMACandle.mA30MinTypicalShortClose = prevMa.MovingAverages30Min[thisInput.var13 - 1].movingAverage.Close;
                    //model.candles.prevMACandle = await CreateLiveCandle(the_db, thisInput.var1, thisInput.var3, thisInput.var2, thisInput.var13, getStartDate, epicName, minute_container, TicksContainer, true, false);
                    //watch.Stop();
                    //Console.WriteLine(DateTime.Now.ToString("o") + " - Get prevMA candle - Time taken = " + watch.ElapsedMilliseconds);

                    // watch.Restart();
                    //PlaceDeal("long", 0.5, 42);
                    //model.sellLong = false;
                    //TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "RunCode", "Running ProTrendCode");
                    model.RunProTrendCodeV2(model.candles);
                    //TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "RunCode", "ProTrendCode finished");
                    //model.buyLong = true;
                    //  model.sellLong = true;
                    // force a long
                    ...
                    Console.WriteLine("trade sub key = " + _tradeSubscriptionStk.ToString());
                    ...
                    if (param != "DEBUG")
                {
                    ...
                    else
                    {
                        if (model.sellLong)
                        {
                                TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "RunCode", "SellLong");
                                //bool sellOK = await model.thisModel.sell(model.candles.currentCandle.candleStart, model.candles.currentCandle.candleData.FirstOffer, model.candles.currentCandle.candleData.FirstBid, false, model.modelLogs.logs[0], modelVar);
                                Console.WriteLine("SellLong activated");
                                CloseDeal("long", model.modelVar.quantity, this.currentTrade.dealId);
                    ...
                    else
                    {
                        if (model.buyShort)
                            {
                                Console.WriteLine("BuyShort activated");
                                TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "RunCode", "BuyShort)");
                                CloseDeal("short", model.modelVar.quantity, this.currentTrade.dealId);
 
                        }
                    }
                    if (model.longOnmarket)
                    {
                        if (IGModels.clsCommonFunctions.Dbl2DP((double)model.thisModel.currentTrade.buyPrice - model.thisModel.currentTrade.stopLossValue) != IGModels.clsCommonFunctions.Dbl2DP((double)this.currentTrade.stopLevel))
                        {
                                Console.WriteLine("EditLong activated");
                                TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "RunCode", "Edit Long Deal");
                    ...
                    {
                        if (IGModels.clsCommonFunctions.Dbl2DP((double)model.thisModel.currentTrade.sellPrice + model.thisModel.currentTrade.stopLossValue) != IGModels.clsCommonFunctions.Dbl2DP((double)this.currentTrade.stopLevel))
                        {
                                Console.WriteLine("EditShort activated");
                                TradingBrain.Models.clsCommonFunctions.SaveLog("Info", "RunCode", "Edit Short Deal");
                                if (model.modelVar.breakEvenVar == 1)
                                {
                ...
                if (model.modelLogs.logs.Count() > 0)
                {
                    ...
                    Container logContainer = the_db.GetContainer("ModelLogs");
                ...
                bigWatch.Stop();
                //Console.WriteLine(DateTime.Now.ToString("o") + " - Completed run - Time taken = " + bigWatch.ElapsedMilliseconds);
            }
        ...
        private void OnMarketUpdate(object sender, UpdateArgs<L1LsPriceData> e)
        {
            try
            {
                ...
                //clsCandleUpdate objUpdate = new clsCandleUpdate();
                currentCandle.Epic = epic;
                currentCandle.Bid = wlmUpdate.Bid ?? 0;
                currentCandle.Change = wlmUpdate.Change ?? 0;
                currentCandle.ChangePct = wlmUpdate.ChangePct ?? 0;
                currentCandle.High = wlmUpdate.High ?? 0;
                currentCandle.Low = wlmUpdate.Low ?? 0;
                currentCandle.MarketDelay = wlmUpdate.MarketDelay ?? 0;
                currentCandle.MarketState = wlmUpdate.MarketState ?? "";
                currentCandle.MidOpen = wlmUpdate.MidOpen ?? 0;
                currentCandle.Offer = wlmUpdate.Offer ?? 0;
                currentCandle.UpdateTime = wlmUpdate.UpdateTime ?? "";


                //if (this.the_db != null && this.the_chart_container != null)
                //{
                //    objUpdate.Add(the_db, the_chart_container).DiscardTask();
                //}
            ...
            catch (Exception ex)
            {
                ...
                log.Log_Message = ex.ToString();
                log.Log_Type = "Error";
                log.Log_App = "OnMarketUpdate";
                log.Save();
            }
        }

        private void OnTickUpdate(object sender, UpdateArgs<ChartTickData> e)
        {

            var epic = e.ItemName.Replace("L1:", "").Replace("CHART:", "").Replace(":TICK", "");
            try
            {
                var wlmUpdate = e.UpdateData;
                ...
                //AddStatusMessage("TICK COUNT:" + _lngTickCount.ToString());

                if (_lngTickCount % 100 == 0)
                {
                    // output stats for each epic
                    //AddStatusMessage("**********" + DateTime.UtcNow.ToString() + " Ticks:" + _lngTickCount.ToString());

                    foreach (clsEpicList item in this.EpicList)
                    {
                        //AddStatusMessage("TICKS:" + item.Epic + " Count:" + item.counter.ToString());
                    }
                    //AddStatusMessage("**********");

                }
                // handle max value for counter, dont let it go near the max value
                if (_lngTickCount > (long.MaxValue - 1000))
                {
                    _lngTickCount = 0;
                }

                if (wlmUpdate.Bid != 0 && wlmUpdate.Offer != 0)
                {
                    //clsChartUpdate objUpdate = new clsChartUpdate();
                    if (wlmUpdate.Bid != null && wlmUpdate.Offer != null)
                    {
                        currentTick.Epic = epic;
                        currentTick.Bid = wlmUpdate.Bid ?? 0;
                        currentTick.Offer = wlmUpdate.Offer ?? 0;
                        currentTick.LTP = wlmUpdate.LastTradedPrice ?? 0;
                        currentTick.LTV = wlmUpdate.LastTradedVolume ?? 0;
                        currentTick.TTV = wlmUpdate.IncrimetalTradingVolume ?? 0;
                        if (wlmUpdate.UpdateTime != null)
                        {
                            currentTick.UTM = (DateTime)wlmUpdate.UpdateTime;
                        }
                        currentTick.DAY_OPEN_MID = wlmUpdate.DayMidOpenPrice ?? 0;
                        currentTick.DAY_NET_CHG_MID = wlmUpdate.DayChange ?? 0;
                        currentTick.DAY_PERC_CHG_MID = wlmUpdate.DayChangePct ?? 0;
                        currentTick.DAY_HIGH = wlmUpdate.DayHigh ?? 0;
                        currentTick.DAY_LOW = wlmUpdate.DayLow ?? 0;
                    }
                    ////AddStatusMessage(objUpdate.Epic + " BId:" + objUpdate.Bid.ToString());


                    //if (this.the_db != null && this.the_chart_container != null)
                    //{
                    //    objUpdate.Add(the_db, this.the_chart_container).DiscardTask();
                    //}
        ...
        public async Task<int> WaitForChanges()
        {
            ...
            while (DateTime.UtcNow < dtMax || 1 == 1)
            {
                ...
                DateTime dtNow = DateTime.UtcNow;
                //Console.WriteLine(dtNow.ToString("o") + " Sleeping....");
            }
            UnsubscribeFromWatchlistInstruments();
            UnsubscribefromTradeSubscription();
            }
        ...
        public async void setupMessaging()
        {
            ...
            hubConnection = new HubConnectionBuilder()
                .WithUrl("https://igmessaging.azurewebsites.net/Api", (HttpConnectionOptions options) => options.Headers.Add("userid", "TradingBrain"))
...

                {
                    ...
                    switch (obj.messageType)
                    {
                        case "Ping":
                            ...
                            clsCommonFunctions.SendMessage(obj.messageValue, "Ping", "OK");
                            break;
            ...
            hubConnection.Reconnecting += error =>
            {
                ...
                Console.WriteLine("Messaging connection lost, retrying");
                return Task.CompletedTask;
            };
            hubConnection.Reconnected += connectionId =>
            {
                Debug.Assert(hubConnection.State == HubConnectionState.Connected);
                Console.WriteLine("Messaging connection reconnected");
        ...
        //    try
        //    {
        //        foreach (string item in watchlistItems)
        //        {
        //            clsCommonFunctions.AddStatusMessage("Epic:" + item);
        //        }
        ...
        //            clsCommonFunctions.AddStatusMessage("Successfully subscribed to epics");
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        //        clsCommonFunctions.AddStatusMessage("Exception when trying to subscribe to Watchlist l1 prices: " + ex.Message);

        //        Log log = new Log();
        //        log.Log_Message = ex.ToString();
        //        log.Log_Type = "Error";
        //        log.Log_App = "SubscribeL1WatchlistPrices";
        //        log.Save();
        //    }
            ...
            try
            {
                if (igStreamApiClient != null)
                {
                    ...
                    foreach (var epic in chartEpics)
                    {
                        ...
                        clsCommonFunctions.AddStatusMessage("Subscribing to Chart Data (Ticks ): " + ccd.ChartEpic);
                    }

                    _chartSubscribedTableKey = igStreamApiClient.SubscribeToChartTicks(chartEpics, _l1ChartSubscription);
                    clsCommonFunctions.AddStatusMessage("Ticks key: " + _chartSubscribedTableKey.ToString());

                }
            }
            catch (Exception ex)
            {
                clsCommonFunctions.AddStatusMessage("Exception when trying to subscribe to Chart Candle Data: " + ex.Message);
        ...
        private void UnsubscribeFromWatchlistInstruments()
        {
            try
            {
                if ((igStreamApiClient != null) && (_watchlistL1PricesSubscribedTableKey != null) && (LoggedIn))
                {
                    ...
                    clsCommonFunctions.AddStatusMessage("WatchlistsViewModel : Unsubscribing from L1 Prices for Watchlists");
                }
            }
            catch (Exception e)
            {
        ...
        private void UnsubscribefromTradeSubscription()
        {
            try
            {
                if ((_tradeSubscriptionStk != null) && (igStreamApiClient != null))
                {
                    ...
                    Console.WriteLine("Successfully unsubscribed from Trade Subscription");
                }
            }
            catch (Exception e)
            {
        ...
        {
            try {
                clsCommonFunctions.AddStatusMessage("Attempting login");
            ...
            if (igWebApiConnectionConfig != null && igRestApiClient != null && igStreamApiClient != null && Accounts != null)
            {
                ...
                string apiKey = igWebApiConnectionConfig["apikey." + env] ?? "";
                    clsCommonFunctions.AddStatusMessage("User=" + userName + " is attempting to login to environment=" + env);


                if (String.IsNullOrEmpty(userName) || String.IsNullOrEmpty(password) || String.IsNullOrEmpty(apiKey))
                {
                        clsCommonFunctions.AddStatusMessage("Please enter a valid username / password / ApiKey combination in ApplicationViewModel ( Login method )");
                    return;
                }
                    ...
                    if (response && (response.Response != null) && (response.Response.accounts.Count > 0))
                    {
                        ...
                        foreach (var account in response.Response.accounts)
                        {
                            ...
                            Accounts.Add(igAccount);
                                ...
                                clsCommonFunctions.AddStatusMessage("Account:" + igAccount.ClientId + " " + account.accountName);
                        }

                        LoggedIn = true;

                            clsCommonFunctions.AddStatusMessage("Logged in, current account: " + response.Response.currentAccountId);

                        ConversationContext context = igRestApiClient.GetConversationContext();

                            clsCommonFunctions.AddStatusMessage("establishing datastream connection");
                            ...
                            (context.apiKey != null) && (context.xSecurityToken != null) && (context.cst != null))
                        {
                            try
                            {
                                ...
                                {
                                        clsCommonFunctions.AddStatusMessage(String.Format("Connecting to Lightstreamer. Endpoint ={0}",
                                                                        response.Response.lightstreamerEndpoint));
                                ...
                                else
                                {
                                    igStreamApiClient = null;
                                        clsCommonFunctions.AddStatusMessage(String.Format(
                            ...
                            {
                                    clsCommonFunctions.AddStatusMessage(ex.Message);
                            }
                        }
                    }
                    else
                ...
                {
                        clsCommonFunctions.AddStatusMessage("ApplicationViewModel exception : " + ex.Message);
                }
            }
        ...
        public static async Task<ModelMinuteCandle> CreateLiveCandle(Database the_db, int minAvgIndex, int min30AvgIndex, int minAvgIndexShort, int min30AvgIndexShort, DateTime dtDate, string epicName, Container minute_container, Container TicksContainer, bool MAonly, bool createMinRecord)
        {
            //var watch = new System.Diagnostics.Stopwatch();
            ...
            if (!MAonly)
            {
                ...
                if (createMinRecord)
                {
                    ...
                    if (data.Open == 0)
                    {
                        Console.Write(DateTime.Now.ToString("o") + "TickData not found. Creating ...");
                        ...
                        Console.WriteLine(DateTime.Now.ToString("o") + "New tick created. ");
                    ...
                    //bigWatch.Stop();
                    //Console.WriteLine(DateTime.Now.ToString("o") + " - Created minute candle - Time taken = " + bigWatch.ElapsedMilliseconds);
                }
            ...
            CandleMovingAverage min30Avg = await Get_MinuteMovingAverageNum30v1(the_db, minute_container, epic, retCandle.candleStart, min30AvgIndex);
            //bigWatch.Stop();
            //Console.WriteLine(DateTime.Now.ToString("o") + " - min30Avg - Time taken = " + bigWatch.ElapsedMilliseconds);
            //bigWatch.Restart();
            ...
            CandleMovingAverage min30AvgShort = await Get_MinuteMovingAverageNum30v1(the_db, minute_container, epic, retCandle.candleStart, min30AvgIndexShort);

            // bigWatch.Stop();
            //Console.WriteLine(DateTime.Now.ToString("o") + " - min30AvgShort - Time taken = " + bigWatch.ElapsedMilliseconds);
        ...
        public class AvgDates
        {
            ...
            public void GetAvgDates(DateTime now, int i)
            {
                ...
                //Console.WriteLine(i + " ---- Now : " + now + " Start : " + ret.start + " End : " + ret.end);

            }
        }

        public static async Task<CandleMovingAverage> Get_MinuteMovingAverageNum30(Database the_db, Container container, IG_Epic epic, DateTime CandleStart, int num)
        {
        ...
        public static async Task<CandleMovingAverage> Get_MinuteMovingAverageNum30v1(Database the_db, Container container, IG_Epic epic, DateTime CandleStart, int num)
        {
        ...
        public static async Task<CandleMovingAverage> Get_MinuteMovingAverageNum30v2(Database the_db, Container container, IG_Epic epic, DateTime CandleStart, int num)
        {
        ...
        public async Task<tradeItem> GetTradeFromDB(string dealID)
        {
            ...
            try
            {
                Microsoft.Azure.Cosmos.Container container = the_db.GetContainer("TradingBrainTrades");
            ...
            catch (CosmosException de)
            {
                if (de.StatusCode != System.Net.HttpStatusCode.NotFound)
                {
                    Log log = new TradingBrain.Models.Log();
                    log.Log_Message = de.ToString();
                    log.Log_Type = "Error";
